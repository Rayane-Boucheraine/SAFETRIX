...
const StartupForgotPasswordPage = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!email) {
      setMessage("Please enter the company email address.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "Forgot password API error (Startup Flow):",
          response.statusText
        );
      }
    } catch (error) {
      console.error("Forgot password request failed (Startup Flow):", error);
      ...
      <div className="w-full max-w-md p-6 md:p-10 rounded-[16px] border border-white/40 bg-[rgba(64,127,103,0.1)] shadow-[0px_4px_30px_0px_rgba(255,255,255,0.15)] backdrop-blur-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            ...
            className="w-full cursor-pointer bg-green-600 hover:bg-green-700 text-white text-sm font-medium py-2.5 px-4 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#102c1f] focus:ring-green-500 disabled:opacity-60 disabled:cursor-not-allowed" // Dark green/black offset
          >
        ...
        <p className="mt-6 text-center text-xs text-gray-400">
          Remembered your password? {/* --- Link Back to Startup Sign In --- */}
          <Link
