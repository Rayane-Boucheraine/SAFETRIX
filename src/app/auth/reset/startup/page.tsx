// app/auth/forgot-password/startup/page.tsx
"use client";
...
const StartupForgotPasswordPage = () => {
  ...
  >({
    mutationFn: async (payload) => {
      try {
        ...
        // Simulate success to avoid revealing if an email exists
        return { message: "If an account exists, an email will be sent." }; // Simulate success response
      }
    },
    onSuccess: (data) => {
      toast.success(
      ...
      setSubmittedEmail(email); // Store the email that was submitted
      setEmail(""); // Clear the input field
    },
    onError: (error: Error) => {
      // This might only trigger for network errors if we don't re-throw above
      toast.error(error.message || "An unexpected error occurred.");
      setSubmittedEmail(null); // Reset submitted email on error
    },
  });

  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setSubmittedEmail(null); // Reset submission state
    ...
    <div className="bg-[radial-gradient(70.07%_69.22%_at_50%_50%,#195033_6.63%,#080808_100%)] w-full min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md p-6 md:p-10 rounded-[16px] border border-white/40 bg-[rgba(64,127,103,0.1)] shadow-[0px_4px_30px_0px_rgba(255,255,255,0.15)] backdrop-blur-sm">
        ...
        <p className="text-gray-300 mb-6 text-sm text-center">
          Enter your company email below and we&apos;ll send you a link to reset
          it.
        </p>
          ...
          {!submittedEmail ? ( // Show form only if not submitted
            <>
              ...
              <button
                type="submit"
                disabled={forgotPasswordMutation.isPending}
                className="w-full cursor-pointer bg-green-600 hover:bg-green-700 text-white text-sm font-medium py-2.5 px-4 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#1a4530] focus:ring-green-500 disabled:opacity-60 disabled:cursor-not-allowed"
              >
...
export default StartupForgotPasswordPage;
